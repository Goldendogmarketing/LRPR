    ...
    <main className="min-h-screen bg-[#f7f3eb] text-slate-950">
      ...
      <section className="mx-auto grid max-w-7xl gap-8 px-4 py-12 sm:px-6 lg:grid-cols-[1fr_0.82fr] lg:px-8">
        <div>
          ...
          <div className="mt-5 flex flex-wrap items-center gap-3"><StatusBadge status={listing.status} /><span className="text-2xl font-black">{listing.price}</span><span className="rounded-full bg-white px-3 py-1 text-sm font-black">{listing.category}</span></div>
      ...
      <section className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-2 lg:grid-cols-3">{related.map((item) => <ListingCard listing={item} key={item.id} />)}</div>
      </section>
      <InternalLinkHub />
    </main>
  );
